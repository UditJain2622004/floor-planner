...
const defaults = {
  door:{w:4,h:.7,fill:"#ffffff",stroke:"#4a5661"},
  ...
  tv:{w:5.2,h:.8,fill:"#2f363d",stroke:"#20262c"},
...
function parseFtIn(value){
  ...
  let m=raw.match(/^\s*(\d+(?:\.\d+)?)\s*(?:ft|feet|')\s*(?:[-\s]\s*)?(\d+(?:\.\d+)?)?\s*(?:"|in|inch|inches)?\s*$/i);
  if(m){
...
function addObject(type, x=8, y=6){
  ...
  const offset = (state.objects.length % 8) * .5;
...
function buildGeometry(o){
  ...
  const line = (x1,y1,x2,y2,sw=thin,extra={}) =>
    shapes.push(el("line",{x1,y1,x2,y2,stroke,"stroke-width":sw,...extra}));

  const rect = (x,y,ww,hh,fill2=fill,rx=0,sw=thin,extra={}) =>
    shapes.push(el("rect",{x,y,width:ww,height:hh,fill:fill2,stroke,"stroke-width":sw,rx,...extra}));

  const circle = (cx,cy,r,fill2="none",sw=thin,extra={}) =>
    shapes.push(el("circle",{cx,cy,r,fill:fill2,stroke,"stroke-width":sw,...extra}));

  const ellipse = (cx,cy,rx,ry,fill2="none",sw=thin) =>
    shapes.push(el("ellipse",{cx,cy,rx,ry,fill:fill2,stroke,"stroke-width":sw}));

  if(o.type === "wall"){
    rect(0,0,w,h,fill,1,med);
    line(0,h/2,w,h/2,.65,{stroke:"#7f8992"});
  }
    ...
    rect(w*.08,h*.10,w*.84,h*.80,"#ffffff",0,1.0,{stroke:"#8f979e","stroke-dasharray":"4 3"});

    // Small threshold ticks on the sides make orientation obvious.
    line(w*.08,h*.10,w*.08,h*.22,1.1,{stroke:"#6f7880"});
    line(w*.92,h*.10,w*.92,h*.22,1.1,{stroke:"#6f7880"});
  }

  else if(o.type === "bike"){
    const r = Math.min(w,h)*.19;
    circle(w*.22,h*.72,r,"none",med);
    circle(w*.78,h*.72,r,"none",med);

    line(w*.22,h*.72,w*.40,h*.45,med);
    line(w*.40,h*.45,w*.63,h*.72,med);
    line(w*.63,h*.72,w*.22,h*.72,med);
    line(w*.40,h*.45,w*.54,h*.35,med);
    line(w*.54,h*.35,w*.68,h*.32,1.4);
    line(w*.40,h*.45,w*.28,h*.39,1.4);

    // pedals + seat
    circle(w*.43,h*.57,Math.min(w,h)*.06,"none",.9);
    line(w*.29,h*.36,w*.45,h*.36,1.5);
  }

  else if(o.type === "car"){
    rect(w*.08,h*.03,w*.84,h*.94,"#dce3e8",w*.11,med);
    rect(w*.18,h*.16,w*.64,h*.28,"#b9c9d5",w*.035,1.0);
    rect(w*.18,h*.56,w*.64,h*.22,"#d7dfe5",w*.025,1.0);

    circle(w*.18,h*.82,w*.075,"#252b31",.8);
    circle(w*.82,h*.82,w*.075,"#252b31",.8);
    circle(w*.18,h*.18,w*.075,"#252b31",.8);
    circle(w*.82,h*.18,w*.075,"#252b31",.8);

    line(w*.5,h*.44,w*.5,h*.56,1.0);
    line(w*.31,h*.67,w*.69,h*.67,.8);
    line(w*.30,h*.13,w*.70,h*.13,1.0);
  }
    ...
    for(let i=1;i<8;i++){
      line(0,h*i/8,w,h*i/8,.8,{stroke:"#626e78"});
    }

    // Turning stair path / direction.
    line(w*.24,h*.84,w*.24,h*.54,1.8);
    line(w*.24,h*.54,w*.62,h*.54,1.8);
    line(w*.62,h*.54,w*.62,h*.18,1.8);
    ...
    rect(
      ...
      "#fff",0,.8,{stroke:"#7d878f"}
    );

    // Four separate wall bands.
    if(top>0) rect(0,0,w,top,"#dfe3e6",0,1,{stroke:"#68737c"});
    if(bottom>0) rect(0,h-bottom,w,bottom,"#dfe3e6",0,1,{stroke:"#68737c"});
    if(left>0) rect(0,top,left,Math.max(0,h-top-bottom),"#dfe3e6",0,1,{stroke:"#68737c"});
    if(right>0) rect(w-right,top,right,Math.max(0,h-top-bottom),"#dfe3e6",0,1,{stroke:"#68737c"});
  }

  else if(o.type === "dining"){
    rect(w*.22,h*.17,w*.56,h*.66,"#d8c4a7",6,med);
    const cr = Math.min(w,h)*.075;
    for(const [px,py] of [[.10,.25],[.10,.50],[.10,.75],[.90,.25],[.90,.50],[.90,.75]])
      circle(w*px,h*py,cr,"#eef1f2",1.0);

    circle(w*.50,h*.50,Math.min(w,h)*.06,"#c3aa89",.7);
  }

  else if(o.type === "sofa"){
    rect(0,h*.22,w,h*.61,"#c7d1d8",10,med);
    rect(w*.06,h*.05,w*.88,h*.28,"#d9e1e6",9,med);
    rect(w*.01,h*.55,w*.18,h*.30,"#b7c3cb",8,1.0);
    rect(w*.81,h*.55,w*.18,h*.30,"#b7c3cb",8,1.0);
    line(w*.33,h*.28,w*.33,h*.75,.9);
    line(w*.66,h*.28,w*.66,h*.75,.9);
  }

  else if(o.type === "table"){
    rect(w*.08,h*.15,w*.84,h*.70,"#d8c09e",7,med);
    line(w*.20,h*.79,w*.20,h,1.7);
    line(w*.80,h*.79,w*.80,h,1.7);
    line(w*.27,h*.50,w*.73,h*.50,.8);
  }

  else if(o.type === "tv"){
    rect(0,0,w,h*.72,"#333a41",2,med);
    rect(w*.08,h*.10,w*.84,h*.48,"#14181c",1,.8);
    line(w*.42,h*.72,w*.42,h,1.5);
    line(w*.58,h*.72,w*.58,h,1.5);
    line(w*.36,h,w*.64,h,1.7);
  }

  else if(o.type === "kitchen"){
    rect(0,0,w,h,"#d8dde1",2,med);
    rect(w*.05,h*.12,w*.90,h*.76,"#f0f2f3",1,.9);

    // modules
    for(const x of [.28,.52,.72]) line(w*x,h*.12,w*x,h*.88,.8);

    // sink
    rect(w*.68,h*.22,w*.22,h*.42,"#fff",2,.9);
    ellipse(w*.79,h*.43,w*.065,h*.10,"none",.8);
    shapes.push(el("path",{d:`M ${w*.79} ${h*.22} q 0 -${h*.10} ${w*.06} 0`,fill:"none",stroke,"stroke-width":1.1}));

    // hob
    for(const x of [.18,.39])
      circle(w*x,h*.45,Math.min(w,h)*.055,"none",.8);
  }
    ...
    rect(w*.07,h*.06,w*.86,h*.23,"#f7f9fa",4,.9);
    rect(w*.10,h*.09,w*.30,h*.16,"#fff",2,.8);
    rect(w*.60,h*.09,w*.30,h*.16,"#fff",2,.8);
    line(w*.07,h*.34,w*.93,h*.34,.9);
    line(w*.07,h*.71,w*.93,h*.71,.9);

    // two side rails / wooden outline feel
    line(w*.04,h*.04,w*.04,h*.96,.8,{stroke:"#71808b"});
    line(w*.96,h*.04,w*.96,h*.96,.8,{stroke:"#71808b"});
  }

  else if(o.type === "basin"){
    ellipse(w*.50,h*.56,w*.39,h*.29,"#eef4f7",med);
    shapes.push(el("path",{d:`M ${w*.42} ${h*.28} Q ${w*.50} ${h*.07} ${w*.58} ${h*.28}`,fill:"none",stroke,"stroke-width":1.5}));
    circle(w*.50,h*.58,Math.min(w,h)*.04,"#fff",.8);
    line(w*.50,h*.10,w*.50,h*.20,.8);
  }

  else if(o.type === "toilet"){
    rect(w*.18,0,w*.64,h*.20,"#eef2f4",4,.9);
    ellipse(w*.50,h*.59,w*.28,h*.32,"#f9fafb",med);
    shapes.push(el("path",{d:`M ${w*.34} ${h*.82} Q ${w*.50} ${h*.94} ${w*.66} ${h*.82}`,fill:"none",stroke,"stroke-width":.8}));
    line(w*.50,h*.27,w*.50,h*.42,.9);
  }

  else if(o.type === "washing"){
    rect(0,0,w,h,"#e8ecef",3,med);
    rect(w*.08,h*.07,w*.84,h*.14,"#f9fafb",1,.8);
    circle(w*.50,h*.57,w*.27,"#f8fafb",med);
    circle(w*.50,h*.57,w*.18,"#dbe5eb",.9);
    circle(w*.78,h*.14,Math.min(w,h)*.035,"#5a656e",.7);
  }

  else if(o.type === "plant"){
    rect(w*.27,h*.58,w*.46,h*.30,"#c5ae97",4,.9);
    ...
    for(const [px,py,rx,ry,rot] of [
      ...
      const leaf = el("ellipse",{
        cx:w*px,cy:h*py,rx:w*rx,ry:h*ry,
        fill:"#a9c8a6",stroke:"#57715a","stroke-width":.8
      });
...
function nearestWall(point, maxDist=.75){
  let best=null;
...
function measurementInfo(o){
  ...
  const label=mode==="inner"
    ? (inferred ? "INNER / CLEAR" : "INNER")
    : mode==="outer"
      ? (inferred ? "OUTER / OVERALL" : "OUTER")
      : "CENTERLINE";
...
function dimensionModeLabel(mode){
  return mode==="inner" ? "INNER / CLEAR" :
         mode==="outer" ? "OUTER / OVERALL" :
         "CENTERLINE";
}
...
function drawObject(o){
  ...
  if(o.label || o.type === "room"){
    const label = el("text",{
      ...
      y:(b.h*.42)+(Number(o.labelOffsetY)||0)*canvas.scale,
      "text-anchor":"middle",
      "dominant-baseline":"middle",
      fill:"#34404a",
      "font-size":Math.max(10,Math.min(16,b.w*.05)),
    ...
    if(o.type==="room"){
      ...
      const dim=el("text",{
        ...
        y:(b.h*.57)+(Number(o.dimensionOffsetY)||0)*canvas.scale,
        "text-anchor":"middle",
        "dominant-baseline":"middle",
        fill:"#46525d",
        "font-size":16,
  ...
  if(showDefaultDimension){
    ...
    }else if(o.type==="room"){ label.textContent="";
    }else if(o.type==="circle"){
...
function render(){
  ...
  const ordered=[...state.objects].sort((a,b)=>renderOrder(a)-renderOrder(b));
...
function niceGridStep(zoom){
  ...
  const candidates=[.25,.5,1,2,5,10,20,50,100,200,500,1000,2000];
...
function renderGrid(){
  ...
  for(let x=startX;x<=endX; x+=step,index++){
    ...
    overlayGrid.appendChild(el("line",{
      x1:x*canvas.scale,
  ...
  if(camera.x<=0 && camera.x+camera.w>=0){
    overlayGrid.appendChild(el("line",{
      x1:0,y1:camera.y*canvas.scale,x2:0,y2:(camera.y+camera.h)*canvas.scale,
      stroke:"#bcc5cc","stroke-width":"1.4"
    }));
  }
  if(camera.y<=0 && camera.y+camera.h>=0){
    overlayGrid.appendChild(el("line",{
      x1:camera.x*canvas.scale,y1:0,x2:(camera.x+camera.w)*canvas.scale,y2:0,
      stroke:"#bcc5cc","stroke-width":"1.4"
    }));
  }
}
...
function axisAlignedObject(o){
  return Math.abs((o.rotation||0)%90) < 0.001 || Math.abs(Math.abs((o.rotation||0)%90)-90)<0.001;
}
...
function gapBetweenObjects(aObj,bObj){
  ...
  if(dx>0){
    ...
    const y=yTop<=yBottom ? (yTop+yBottom)/2 : (Math.max(a.top,b.top)+Math.min(a.bottom,b.bottom))/2;
  ...
  if(dy>0){
    ...
    const x=xLeft<=xRight ? (xLeft+xRight)/2 : (Math.max(a.left,b.left)+Math.min(a.right,b.right))/2;
...
function nearestGapsFor(o){
  ...
  for(const other of state.objects){
    ...
    if(gaps.horizontal && gaps.horizontal.distance>=0 &&
       (!bestH || gaps.horizontal.distance<bestH.distance)){
      bestH={...gaps.horizontal,other};
    }

    if(gaps.vertical && gaps.vertical.distance>=0 &&
...
function drawGapDimension(g, d, horizontal=true){
  ...
  if(horizontal){
    ...
    g.appendChild(el("line",{x1:x1*canvas.scale,y1:y*canvas.scale,x2:x2*canvas.scale,y2:y*canvas.scale,stroke:color,"stroke-width":1.25}));
    g.appendChild(el("line",{x1:x1*canvas.scale,y1:(y-offset)*canvas.scale,x2:x1*canvas.scale,y2:(y+offset)*canvas.scale,stroke:color,"stroke-width":1.05}));
    g.appendChild(el("line",{x1:x2*canvas.scale,y1:(y-offset)*canvas.scale,x2:x2*canvas.scale,y2:(y+offset)*canvas.scale,stroke:color,"stroke-width":1.05}));
    ...
    g.appendChild(el("line",{x1:x*canvas.scale,y1:y1*canvas.scale,x2:x*canvas.scale,y2:y2*canvas.scale,stroke:color,"stroke-width":1.25}));
    g.appendChild(el("line",{x1:(x-offset)*canvas.scale,y1:y1*canvas.scale,x2:(x+offset)*canvas.scale,y2:y1*canvas.scale,stroke:color,"stroke-width":1.05}));
    g.appendChild(el("line",{x1:(x-offset)*canvas.scale,y1:y2*canvas.scale,x2:(x+offset)*canvas.scale,y2:y2*canvas.scale,stroke:color,"stroke-width":1.05}));
    ...
    t.textContent=ftIn(d.distance);
    t.setAttribute("transform",`rotate(-90 ${(x+.18)*canvas.scale} ${mid*canvas.scale})`);
    g.appendChild(t);
  }
}
...
function drawPairDimensions(first,second){
  ...
  if(!gaps.horizontal && !gaps.vertical){
    ...
    const a=objectBounds(first), b=objectBounds(second);
    ...
    if(xGap===0 && yGap===0){
      const t=el("text",{
        x:((Math.max(a.left,b.left)+Math.min(a.right,b.right))/2)*canvas.scale,
        y:((Math.max(a.top,b.top)+Math.min(a.bottom,b.bottom))/2)*canvas.scale,
        "text-anchor":"middle",
        fill:"#7c3aed",
        "font-size":16,
        "font-weight":"700",
  ...
  const label=el("text",{
    ...
    y:(Math.min(a.top,b.top)-.9)*canvas.scale,
    fill:"#7c3aed",
    "font-size":12,
    "font-weight":"700",
    "paint-order":"stroke",
...
function drawSelection(){
  ...
  if(o.type==="measurement"){
    ...
    g.appendChild(el("line",{
      x1:a.x*canvas.scale,y1:a.y*canvas.scale,
      x2:b.x*canvas.scale,y2:b.y*canvas.scale,
  ...
  g.appendChild(el("polygon",{
    points:pts.map(p=>`${p.x*canvas.scale},${p.y*canvas.scale}`).join(" "),
    fill:"none",
    stroke:"#2563eb",
    "stroke-width":1.5,
  ...
  for(const [p,cursor,index] of handles){
    ...
    h.setAttribute("transform",`rotate(${o.rotation||0} ${p.x*canvas.scale} ${p.y*canvas.scale})`);
...
function beginRoomTextDrag(e,id,kind){
  ...
  const ox=kind==="label" ? (Number(o.labelOffsetX)||0) : (Number(o.dimensionOffsetX)||0);
  const oy=kind==="label" ? (Number(o.labelOffsetY)||0) : (Number(o.dimensionOffsetY)||0);
...
function renderMeasurementDraft(p){
  ...
  g.appendChild(el("line",{x1:sx,y1:sy,x2:ex,y2:ey,stroke:"#2563eb","stroke-width":1.5}));
  g.appendChild(el("line",{x1:sx-dy/len*6,y1:sy+dx/len*6,x2:sx+dy/len*6,y2:sy-dx/len*6,stroke:"#2563eb","stroke-width":1.2}));
  g.appendChild(el("line",{x1:ex-dy/len*6,y1:ey+dx/len*6,x2:ex+dy/len*6,y2:ey-dx/len*6,stroke:"#2563eb","stroke-width":1.2}));

  const t=el("text",{x:(sx+ex)/2,y:(sy+ey)/2-8,"text-anchor":"middle",fill:"#2563eb","font-size":12,"font-weight":"700"});
  const draftObj={
    x:measureDraft.startX,
    y:measureDraft.startY,
    w:Math.hypot(p.x-measureDraft.startX,p.y-measureDraft.startY),
    h:.15,
    rotation:Math.atan2(p.y-measureDraft.startY,p.x-measureDraft.startX)*180/Math.PI,
...
function endMeasurement(e){
  ...
  const lineW=Math.max(.05,Math.abs(endX-startX));
...
function findSmartSnap(o,nx,ny){
  ...
  const tolerance=.35; // 4.2 inches: enough to catch a wall/room while dragging.
...
function onMeasurementResizeMove(e){
  ...
  const fixedX=measurementResize.endpointIndex===1
  ...
  if(measurementResize.endpointIndex===1){
    ...
    o.w=Math.max(.05,Math.abs(nextX-fixedX));
  }else{
    o.x=Math.min(nextX,fixedX);
    o.w=Math.max(.05,Math.abs(fixedX-nextX));
  }
...
function onPointerMove(e){
  if(drag){
    ...
    if(drag.kind === "move"){
      ...
      const center0={x:drag.origX+drag.origW/2,y:drag.origY+drag.origH/2};
      ...
      o.x=worldCenter.x-(o.w/2)*Math.cos((o.rotation||0)*Math.PI/180)+(o.h/2)*Math.sin((o.rotation||0)*Math.PI/180);
      o.y=worldCenter.y-(o.w/2)*Math.sin((o.rotation||0)*Math.PI/180)-(o.h/2)*Math.cos((o.rotation||0)*Math.PI/180);
...
function clampCamera(){
  ...
  camera.w=Math.max(.4,Number(camera.w)||40);
  camera.h=Math.max(.4,Number(camera.h)||30);
...
function zoomAt(clientX,clientY,factor){
  ...
  const newW=Math.max(.4,oldW*factor);
...
function buildPreviewGeometry(o){
  const w=100,h=62;
  const stroke=o.stroke, fill=o.fill;
  const shapes=[];
  const line=(x1,y1,x2,y2,sw=1)=>shapes.push(el("line",{x1,y1,x2,y2,stroke,"stroke-width":sw}));
  const rect=(x,y,ww,hh,fill2=fill,rx=1,sw=1)=>shapes.push(el("rect",{x,y,width:ww,height:hh,fill:fill2,stroke,"stroke-width":sw,rx}));
  const circle=(cx,cy,r,fill2="none",sw=1)=>shapes.push(el("circle",{cx,cy,r,fill:fill2,stroke,"stroke-width":sw}));

  if(o.type==="wall"){
    rect(4,28,92,10,fill,1,2); line(4,33,96,33,.7);
  }else if(o.type==="door"){
    rect(12,12,76,40,"#fff",0,1,{"stroke":"#8f979e","stroke-dasharray":"4 3"});
    line(12,12,12,20,1);
    line(88,12,88,20,1);
  }else if(o.type==="bike"){
    circle(25,42,10,"none",2);circle(75,42,10,"none",2);
    line(25,42,42,25,2);line(42,25,63,42,2);line(63,42,25,42,2);
    line(42,25,55,19,2);line(55,19,69,17,1.3);line(40,18,54,18,1.4);
  }else if(o.type==="car"){
    rect(22,7,56,54,"#dce3e8",8,2);
    rect(29,16,42,15,"#b9c9d5",3,1);
    rect(29,38,42,11,"#d7dfe5",2,1);
    circle(29,49,5,"#252b31",.8);circle(71,49,5,"#252b31",.8);
  }else if(o.type==="stairs"){
    for(let i=1;i<8;i++)line(4,4+i*54/8,96,4+i*54/8,.8);
    line(28,53,28,32,2);line(28,32,62,32,2);line(62,32,62,14,2);
    shapes.push(el("path",{d:"M62 14 l-7 8 M62 14 l7 8",fill:"none",stroke,"stroke-width":1.7}));
  }else if(o.type==="room"){
    rect(4,4,92,54,"#f7fafb",0,2);rect(8,8,84,46,"none",0,.8);
    const t=el("text",{x:50,y:35,"text-anchor":"middle",fill:"#35414a","font-size":10,"font-weight":"700"});
    t.textContent="ROOM";shapes.push(t);
  }else if(o.type==="dining"){
    rect(28,17,44,36,"#d8c4a7",4,2);
    for(const [x,y] of [[16,22],[16,35],[16,48],[84,22],[84,35],[84,48]])circle(x,y,5,"#eef1f2",1);
  }else if(o.type==="sofa"){
    rect(8,20,84,31,"#c7d1d8",5,2);rect(14,10,72,15,"#d9e1e6",4,2);
    line(36,23,36,46,.8);line(64,23,64,46,.8);
  }else if(o.type==="table"){
    rect(17,14,66,36,"#d8c09e",4,2);line(28,49,28,62,1.7);line(72,49,72,62,1.7);
  }else if(o.type==="tv"){
    rect(8,10,84,35,"#333a41",2,2);rect(15,17,70,21,"#14181c",1,.8);
    line(43,45,43,60,1.5);line(57,45,57,60,1.5);line(38,60,62,60,1.5);
  }else if(o.type==="kitchen"){
    rect(5,8,90,46,"#d8dde1",2,2);
    for(const x of [30,52,72]) line(x,13,x,49,.8);
    rect(69,17,20,22,"#fff",2,.8);circle(79,28,4.2,"none",.8);
    circle(21,28,3.5,"none",.8);circle(38,28,3.5,"none",.8);
  }else if(o.type==="rect"){
    rect(5,6,90,50,fill,0,2);
  }else if(o.type==="circle"){
    shapes.push(el("ellipse",{cx:50,cy:31,rx:45,ry:24,fill,stroke,"stroke-width":2}));
  }else if(o.type==="bed"){
    rect(9,5,82,54,"#d4e3ec",4,2);
    rect(15,8,70,13,"#f7f9fa",2,.8);rect(19,10,24,9,"#fff",1,.7);rect(57,10,24,9,"#fff",1,.7);
    line(15,24,85,24,.8);line(15,44,85,44,.8);
  }else if(o.type==="basin"){
    shapes.push(el("ellipse",{cx:50,cy:36,rx:35,ry:18,fill:"#eef4f7",stroke,"stroke-width":2}));
    shapes.push(el("path",{d:"M42 18 Q50 7 58 18",fill:"none",stroke,"stroke-width":1.4}));
    circle(50,36,3.2,"#fff",.7);
  }else if(o.type==="toilet"){
    rect(30,5,40,12,"#eef2f4",2,.8);
    shapes.push(el("ellipse",{cx:50,cy:37,rx:22,ry:17,fill:"#f9fafb",stroke,"stroke-width":2}));
  }else if(o.type==="washing"){
    rect(9,5,82,54,"#e8ecef",3,2);rect(17,10,66,8,"#f9fafb",1,.8);
    circle(50,36,14,"#f8fafb",2);circle(50,36,9,"#dbe5eb",.8);
  }else if(o.type==="plant"){
    rect(32,39,36,17,"#c5ae97",2,.8);
    for(const [x,y,rx,ry] of [[36,29,8,12],[50,18,9,14],[64,29,8,12],[50,31,7,10]])
      shapes.push(el("ellipse",{cx:x,cy:y,rx,ry,fill:"#a9c8a6",stroke:"#57715a","stroke-width":.7}));
  }
...
function setPropNum(id,key){
  ...
  if(o.type==="room" && (key==="w" || key==="h")){
    ensureRoomWalls(o);
    if(key==="w"){
      o.w=Math.max(.25,value+o.leftWall+o.rightWall);
    }else{
      o.h=Math.max(.25,value+o.topWall+o.bottomWall);
    }
  }else{
    o[key]=Math.max(.25,value);
  }
...
function migrateImportedState(raw){
  ...
  // 3) plain {objects:[...], ...}
  const incoming = raw && raw.state && typeof raw.state==="object"
    ? raw.state
    : raw;
  ...
  const migrated={
    ...
    snap:0,
    background:typeof incoming.background==="string" ? incoming.background : "#ffffff",
...
function cameraForImportedContent(){
  ...
  const pad=Math.max(2,Math.max(bounds.w,bounds.h)*.15);
  return {
    x:bounds.x-pad,
    y:bounds.y-pad,
  ...
  reader.onload=()=>{
    try{
      ...
      const importedState={
        ...
        dimensionRef:["inner","outer","center"].includes(incomingState.dimensionRef)
      ...
      importedState.objects=importedState.objects.map(source=>{
        ...
        if(!Number.isFinite(copy.w) || copy.w<=0) copy.w=Number(base.w)||1;
        if(!Number.isFinite(copy.h) || copy.h<=0) copy.h=Number(base.h)||1;
...
function exportPNG(){
  ...
  img.onload=()=>{
    ...
    const outW=Math.max(800,Math.min(4000,Math.round(bounds.w*canvas.scale)));
    const outH=Math.max(600,Math.min(4000,Math.round(bounds.h*canvas.scale)));
    c.width=outW;
    c.height=outH;
...
svgViewport.addEventListener("wheel",e=>{
  ...
  if(e.ctrlKey || e.metaKey || e.altKey){
    ...
    zoomAt(e.clientX,e.clientY,clamp(factor,.95,1.05));
    return;
  }
...
document.addEventListener("keydown",e=>{
  if((e.ctrlKey||e.metaKey) && e.key.toLowerCase()==="c" &&
     selected() && !["INPUT","TEXTAREA"].includes(document.activeElement.tagName)){
    e.preventDefault();
  ...
  if((e.ctrlKey||e.metaKey) && e.key.toLowerCase()==="v" &&
     clipboardObject && !["INPUT","TEXTAREA"].includes(document.activeElement.tagName)){
    e.preventDefault();
    pushHistory();
  ...
  if(e.key==="ArrowLeft"){
    e.preventDefault();pushHistory();o.x-=.5;normalizeObject(o);render();
  }
  if(e.key==="ArrowRight"){
    e.preventDefault();pushHistory();o.x+=.5;normalizeObject(o);render();
  }
  if(e.key==="ArrowUp"){
    e.preventDefault();pushHistory();o.y-=.5;normalizeObject(o);render();
  }
  if(e.key==="ArrowDown"){
    e.preventDefault();pushHistory();o.y+=.5;normalizeObject(o);render();
  }
...
[["propX","x"],["propY","y"],["propW","w"],["propH","h"]].forEach(([id,key])=>{
  document.getElementById(id).addEventListener("change",()=>setPropNum(id,key));
});
...
roomTopWall.addEventListener("change",()=>setRoomWallThickness(roomTopWall,"topWall"));
roomRightWall.addEventListener("change",()=>setRoomWallThickness(roomRightWall,"rightWall"));
roomBottomWall.addEventListener("change",()=>setRoomWallThickness(roomBottomWall,"bottomWall"));
roomLeftWall.addEventListener("change",()=>setRoomWallThickness(roomLeftWall,"leftWall"));
